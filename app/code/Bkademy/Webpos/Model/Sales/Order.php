...
class Order extends \Magento\Sales\Model\Order implements \Magento\Sales\Model\EntityInterface, OrderInterface
{
    ...
    public function getWebposStaffName(){
        if(!$this->getData(OrderInterface::WEBPOS_STAFF_NAME)){
            if($staffId = $this->getData(OrderInterface::WEBPOS_STAFF_ID)){
                return $this->_objectManager->create('Bkademy\Webpos\Model\Staff\Staff')
     ...
     *
     * @param \Bkademy\Webpos\Api\Data\Payment\OrderPaymentInterface[] $webposOrderPayments
     * @return $this
     */
     ...
     *
     * @return \Bkademy\Webpos\Api\Data\Payment\OrderPaymentInterface[]|null Webpos staff name.
     */
     ...
     *
     * @param \Bkademy\Webpos\Api\Data\Checkout\ItemsInfoBuyInterface $itemsInfoBuy
     * @return $this
     */
     ...
     *
     * @return \Bkademy\Webpos\Api\Data\Checkout\ItemsInfoBuyInterface.
     */
    public function getItemsInfoBuy(){
        return;
        $itemsBuyRequest = $this->_objectManager->create('Bkademy\Webpos\Api\Data\Checkout\ItemsInfoBuyInterface');
        $items = $this->getAllVisibleItems();
        if(count($items) > 0){
            $itemsInfoBuy = [];
            foreach ($items as $item) {
                $labels = [];
                $itemInfo = $this->_objectManager->create('Bkademy\Webpos\Api\Data\Checkout\InfoBuyInterface');
